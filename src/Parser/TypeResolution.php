 ...
 *
 * A function type nested inside another one's parameters or return type—however many lists, Options, or struct
 * fields deep—can carry a binder of its own: it is then a self-contained, rank-1-polymorphic value in its own right,
 * opaque to the signature it sits inside—see {@see Signature::hasOwnBinder()}. $typeVariables only ever grows once
 * per {@see self::resolveSignature()} call, when that function type's own binder is resolved, so a nested `fn<...>`
 * never sees the names an enclosing one declared and can freely reuse them, the same way any two unrelated
 * signatures can.
 *
 * @internal
...
final class TypeResolution
{
     ...
     * binder of its own too, but not to reuse a name a binder further out already declared: $typeVariables is passed
     * to $inner below and reaches that nested resolution unchanged, so {@see self::checkTypeVariable()} rejects there
     * any name this binder -- or any binder further out -- has already declared, since the inner one would otherwise
     * shadow the outer variable rather than name a fresh one. A genuinely fresh name is a self-contained signature in
     * its own right, opaque to this one, same as {@see Signature::hasOwnBinder()} treats it everywhere else.
     *
     ...
     * shadowing an enclosing variable is not allowed, and a name neither binder has used is free to introduce
